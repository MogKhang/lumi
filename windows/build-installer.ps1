...
if ($HasX64) {
    ...
    $X64Portable = Join-Path $ResolvedOutput "lumi-windows-x64-portable.7z"
...
if ($HasArm64) {
    ...
    $Arm64Portable = Join-Path $ResolvedOutput "lumi-windows-arm64-portable.7z"
...
#define Publisher "mogkhang"
#define ExeName "lumi.exe"
...
AllowNoIcons=yes
OutputDir=.
OutputBaseFilename=lumi-windows-installer
Compression=lzma
...
Write-Host "Installer:        $(Join-Path $ResolvedOutput 'lumi-windows-installer.exe')" -ForegroundColor White
