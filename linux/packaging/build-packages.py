...
def generate_icons():
    ...
    source = PROJECT_ROOT / "assets/lumi.png"

    for size in ICON_SIZES:
        dest_dir = SCRIPT_DIR / f"icons/{size}x{size}"
        dest_dir.mkdir(parents=True, exist_ok=True)
        dest = dest_dir / "lumi.png"
...
def get_file_mappings() -> list[str]:
    ...
    for size in ICON_SIZES:
        mappings.append(
            f"{SCRIPT_DIR}/icons/{size}x{size}/lumi.png=/usr/share/icons/hicolor/{size}x{size}/apps/lumi.png"
        )
...
def main():
    ...
    for script in ["lumi.sh", "after-install.sh", "after-remove.sh"]:
