...
LazyDatabase _openConnection() {
  return LazyDatabase(() async {
    ...
    // Uses a copy+delete fallback for cross-drive setups (e.g. OneDrive-
    // redirected Documents) so a failed rename never strands startup.
...
/// strand the splash on "Loading servers..." forever. Falls back to
/// copy + delete on any [FileSystemException] and swallows all errors so a
/// failed migration never propagates fatally.
///
...
Future<void> migrateLegacyDesktopDatabase({
  ...
  if (sourceOverride != null) {
    ...
    oldFile = File(p.join(oldFolder.path, 'lumi_downloads.db'));
  }
