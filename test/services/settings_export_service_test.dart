...
import 'package:lumi/services/settings_export_service.dart';
...
void main() {
  ...
  group('buildExportMap type encoding', () {
    test('encodes bool / int / double / string / stringList with type markers', () async {
      ...
      await prefs.setString('name', 'lumi');
      ...
      expect(p['name'], {'type': 'string', 'value': 'lumi'});
      expect(p['list_a'], {
        'type': 'stringList',
        'value': ['x', 'y'],
      });
  ...
  group('round-trip', () {
    test('build → JSON → parse → apply produces the same key/value/type', () async {
      ...
      // `lumi_legacy_prefs_migrated_v1` flag the cache plants. We only assert
    ...
    test('cross-user round-trip: alice exports → bob imports → keys land under bob', () async {
      ...
      // The cache plants `lumi_legacy_prefs_migrated_v1` on first init, so
